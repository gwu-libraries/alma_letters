...
<xsl:choose>
		...
		<xsl:attribute name="style">
			...
			<xsl:for-each select="notification_data/user_for_printing/*[starts-with(local-name(), 'address') and text()]">
					<tr>
						<td>
		...
		<xsl:attribute name="style">
			...
			<tr><td><xsl:value-of select="user_address_list/user_address/city"/>,&#160;
			<xsl:value-of select="user_address_list/user_address/state_province"/>&#160;
			<xsl:value-of select="user_address_list/user_address/postal_code"/></td></tr>
